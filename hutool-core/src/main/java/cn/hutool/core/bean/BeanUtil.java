...
import cn.hutool.core.bean.copier.BeanCopyConsumer;
...
//		}
	 ...
	 * @since 5.4.0
	 */
	public static <S, T> T toBeanIgnoreError(S source, Class<T> clazz) {
	 ...
	 * @since 5.4.0
	 */
	public static <S, T> T toBeanIgnoreCase(S source, Class<T> clazz, boolean ignoreError) {
	 ...
	 * @since 5.2.4
	 */
	public static <S, T> T toBean(S source, Class<T> clazz, CopyOptions options) {
	 ...
	 * @since 5.8.0
	 */
	public static <S, T> T toBean(S source, Supplier<T> targetSupplier, CopyOptions options) {
	...
	public static <S, T> T toBean(S source, Supplier<T> targetSupplier, BeanCopyConsumer<TargetProp, S, T, Object> beanCopyConsumer, CopyOptions options) {
		...
		final T target = targetSupplier.get();
		copyProperties(source, target, beanCopyConsumer, options);
		return target;
	}
	 ...
	 * @param ignoreCase 是否忽略大小写
	 */
	public static <S, T> void copyProperties(S source, T target, boolean ignoreCase) {
	 ...
	 * @param copyOptions 拷贝选项，见 {@link CopyOptions}
	 */
	public static <S, T> void copyProperties(S source, T target, CopyOptions copyOptions) {
		if (null == source) {
			return;
		}
	...
	public static <S, T> void copyProperties(S source, T target, BeanCopyConsumer<TargetProp, S, T, Object> beanCopyConsumer, CopyOptions copyOptions) {
		...
		BeanCopier.create(source, target, beanCopyConsumer, ObjectUtil.defaultIfNull(copyOptions, CopyOptions::create)).copy();
	}
